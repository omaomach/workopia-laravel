            ...
            @auth
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('dashboard') }}">
                        ...
                        @else
                            <img src="{{ asset('storage/avatars/default-avatar.png' . Auth::user()->avatar) }}"
                                alt="{{ Auth::user()->name }}" class="w-10 h-10 rounded-full" />
                        @endif
                    </a>
                </div>
                <x-button-link url="/jobs/create" bgClass="bg-red-500" textClass="text-white" icon="edit">Create
                    Job</x-button-link>
            @else
